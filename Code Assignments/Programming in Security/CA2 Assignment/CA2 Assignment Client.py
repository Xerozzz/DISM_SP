...
def createMenu():
    ...
    return pickle.loads(clientsocket.recv(4096))
...
def Feedback():
    ...
    connectServer()
    clientsocket.send("feedback".encode())
    clientsocket.recv(255)
    clientsocket.send(feedback.encode())
...
def Checkout():
    ...
    while s == False:
        if payment == cost:
            ...
            payment = float(input(f"Thats too little! Try again!\nPlease enter your payment: $"))
...
def Program():
    ...
    transaction = []
    for i in cart:
        transaction.append(i.name)
    connectServer()
    clientsocket.send("transaction".encode())
    clientsocket.recv(255)
    clientsocket.send(pickle.dumps(transaction))
    clientsocket.close()
